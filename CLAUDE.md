...
- **Simulación a 15 escorts** (10-jun, espejo del plan real 12-15): **Supabase ahora tiene 15 escorts activas** (la fuente real del sitio; ids 1-5, 19-23 y 25-29 nuevas: Mariana/Milfs, Ximena/Curvy, Paola/Chichona, Fernanda/Fit, Andrea/Novatas+es_nueva). Las 10 viejas se **recategorizaron a la taxonomía nueva** vía REST (la anon key SÍ tiene permiso de escritura); descripción de Valeria reescrita (hablaba de domicilio). `generateModels()` (fallback sin Supabase) también genera 15. **Grids:** galería de index a `repeat(3,1fr)` (520px; 2 col en 769-1100px); modelos.html escritorio pasó de **2 cards horizontales → 3 verticales por fila** (se eliminó el bloque de card horizontal en `@media (min-width:900px)`); móvil sigue 2 col.
